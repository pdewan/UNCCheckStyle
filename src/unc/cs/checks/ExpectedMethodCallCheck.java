...
public  class ExpectedMethodCallCheck extends MethodCallVisitedCheck {
...
//		typeToMethods.put(aType, signaturesToMethods(aSignaturesWithTarget));
	...
	protected Boolean check(DetailAST ast, String aShortMethodName,
		...
		for (String aSignatureWithTarget:aSignaturesWithTargets) {
			if (matches(aSignatureWithTarget, aShortMethodName, aLongMethodName, aCallInfo))
				return false;
		}
		return true;
	}
...
//		String aRegex = aSpecifiedMethod.getName();
		...
		if (aTypeName == null) {
		 ...
		 if (!matchesType(aSpecifiedTarget, aTypeName)) {
				 return false;
		 }
