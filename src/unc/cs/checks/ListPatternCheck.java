...
public abstract  class ListPatternCheck extends ComprehensiveVisitCheck {
...
//				TokenTypes.METHOD_CALL };
    ...
    protected void logMethodNotFound(DetailAST aTreeAST, DetailAST anErroneousAST, String aProperty) {
    ...
    public abstract Boolean checkActualMethod (STType anSTType, String aDeclarePropertyName ) ;
    public Boolean doPendingCheck(DetailAST ast, DetailAST aTreeAST) {
		...
		if (anSTType == null)
			return null;
		STNameable[] anSTNameables = anSTType.getMethods();
...
//			Boolean hasProperty = anSTType.hasActualProperty(aPropertyName);
			Boolean hasProperty = checkActualMethod(anSTType, aPropertyName);
			if (hasProperty == null) return null;
			if (hasProperty) 
				continue;
			logMethodNotFound(aTreeAST, aNameable.getAST(), aPropertyName);
			retVal = false;
