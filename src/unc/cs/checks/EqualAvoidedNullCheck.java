...
//        }
        ...
        String anOperation = method.getText();
        if (!"equals".equals(anOperation) && !"equalsIgnoreCase".equals(anOperation)) {
        	return;
        }
        if (objCalledOn.getType() != TokenTypes.STRING_LITERAL) {
        	return;
        ...
        // comparing a string literal on lhs with non string literal on rhs
        log(methodCall.getLineNo(), methodCall.getColumnNo(),
                MSG_KEY, objCalledOn.getText());
